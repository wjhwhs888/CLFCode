...
const boost::ut::suite<"CLFSearchContent"> tests = [] {
    "恰好 500 结果：head 240 + 省略 20 + tail 240 + 截断标记"_test = [] {
        CLFTest::CLFTestTempDir dir("clf_search_test_");
        writeNeedleFile(dir.path() / "f.txt", 500);

        auto out = searchContent("needle", dir.path().string(), "");
    ...
    "300 结果（<480）：无省略标记，顺序完整"_test = [] {
        CLFTest::CLFTestTempDir dir("clf_search_test_");
        writeNeedleFile(dir.path() / "f.txt", 300);
    ...
    "无匹配：'(no matches)'"_test = [] {
        CLFTest::CLFTestTempDir dir("clf_search_test_");
        writeNeedleFile(dir.path() / "f.txt", 5);

        auto out = searchContent("zzz-no-such", dir.path().string(), "");
