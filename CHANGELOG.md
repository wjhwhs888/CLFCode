...
- **内置文字颜色从未生效**：AI 回复的"● CLFCode:"标签、输入行"❯"前缀等内置颜色长期显示为无色——根因有两层：ANSI 启用开关从未被调用，且界面框架会丢弃文本内嵌的颜色转义。现已改为界面层直接解析着色，文字配色恢复正常显示

### 优化

- **输入内容视觉强化**：用户输入行从灰色 `>` 改为青色加粗 `❯` 前缀（与 AI 回复的"● CLFCode:"青色标签对称），时间戳转灰色低调——多轮长对话翻页时一眼定位输入内容；会话恢复回显同步同款样式
