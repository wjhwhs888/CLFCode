...
- **大任务优雅分阶段**：一轮对话的工具调用达到上限（48 次）不再报"失败"，AI 会总结已完成的工作与剩余工作，输入「继续」即可接着做——上下文完整保留，进度不丢，任务清单保持未完成状态原样衔接
- **等待提示行（Tips）**：请求处理期间输入框上方轮播操作提示（快捷键、命令、使用技巧），完全无响应超过 5 分钟变红提醒可中断——纯提示，不干预 AI 的工作
- **回合结束声明机制**：工具可以声明"本回合到此为止"（本期为机制预留，为后续交互类工具铺路）

### 修复

- **触顶总结报错**：大任务触顶时的进度总结在流式配置下会显示一条 JSON 解析错误，现已修复
...
- **任务清单面板**：AI 创建待办清单后，界面常驻显示任务面板（⏳ 进行中 / ✓ 已完成 / ○ 待办），任务推进实时更新；全部完成时自动在对话流追加完成记录并收起面板
- **会话完整记录**：会话保存从"只保留最后状态"升级为每轮追加记录——每轮对话、每次清单变化、任务全部完成的时刻都有留痕；`/resume` 恢复时可看到每轮对话完成时的清单状态。旧版会话文件永久可读
- **崩溃不丢进度**：任务清单每次变化即时保存——程序被强制关闭或崩溃时，任务进度保留到最近一步，重启后 `/resume` 即可继续
- **续写式恢复**：`/resume` 恢复会话后继续对话，原会话保留为冻结快照，新对话写入独立的续写文件；从任何历史会话（包括正在进行的会话）都能继续
- **上下文自动压缩**：配置开启后（`agent.context_compression`），上下文接近上限时自动生成会话摘要并压缩，长对话不再撑爆窗口；AI 也可主动调用 `compress_context` 工具压缩
- **模型运行时切换**：`/model <模型名>` 随时切换主模型，无需改配置；可按模型设置不同的输出上限（配置 `agent.model_max_tokens`）

### 修复

- **中文路径全链路支持**：系统排查并修复了所有窄字符文件系统 API 的编码泄漏——状态栏工作目录、`/init` 生成的项目规则（路径与标题）、AI 提示词中的工作目录、搜索结果的文件路径、会话列表的中文标题、知识库文件加载、以及可执行文件位于中文目录时的配置加载，现已统一按 UTF-8 处理
...
- **自动自问自答**：修复了粘贴文本注入输入框后、无需用户任何操作即被自动提交的严重问题（表现为 AI 在无人输入时自行发起对话、持续消耗配额）。现在粘贴注入的文本只会停留在输入框等待确认，必须显式按回车才会提交；手打回车提交不受影响
...
- **启动崩溃**：上一版在部分环境下启动即崩溃（修改了配置结构后增量编译残留了旧代码导致的内存越界），现已通过干净重建修复
- **回合结束拖沓**：每轮对话收尾时后台线程要空等最多一秒，现改为即时唤醒，回合结束的界面清理不再卡顿

### 优化

- **测试提速约 20 倍**：内部回归测试从约 30 秒降至约 1.3 秒，后续开发迭代验证更快
...
- **联网能力**：新增 `web_fetch` 工具，AI 可以抓取网页与 API 内容（支持 GET/POST/HEAD、自定义请求头、超时设置）。响应超过 1MB 会截断，二进制内容自动跳过。**该工具不会携带你的 API key 等任何本机凭据**；POST 请求因有远端副作用，即使在自动模式下也会请求确认
- **待办清单**：新增 `todo_write` 工具，AI 可以维护当前任务的待办列表并随时更新状态。清单随会话一起保存，`/resume` 恢复会话时自动带回，不会在你的项目目录里留下额外文件
- **按行读取文件**：`read_file` 新增 `offset` / `limit` 参数，可以只读取文件的指定行范围
- **指定命令工作目录**：`execute_command` 新增 `cwd` 参数

### 安全

- **文件读取范围限制**：`read_file` 现在只能读取工作区内的文件，读取工作区之外的路径会被拒绝（软链接指向外部同样拦截）。如确有需要，可在配置中开启 `agent.allow_absolute_read`
- **超大文件保护**：`read_file` 单文件上限 50MB，超出则拒绝，避免撑爆内存
- **危险命令确认**：执行 `rm -rf`、`del /s`、`git reset --hard`、`shutdown` 等命令时会要求确认，**即使处于自动模式**。可通过配置 `agent.command_allowlist` 为特定命令前缀免除确认。请注意这是防误操作的提示层，不能替代安全模式

### 修复

- **命令结果误判**：`grep` / `rg` / `findstr` / `diff` / `fc` 在"无匹配"或"有差异"时返回退出码 1，此前会被当作命令执行失败，现已按正常结果处理
- **搜索扫到二进制文件**：`search_content` 未指定文件类型时会扫描 `.dll` / `.exe` 等二进制文件，既慢又可能让结果出错；现在默认只搜常见文本类型（仍可用 `fileTypes` 显式指定）
- **搜索结果编码错误**：搜索命中的内容若为 GBK 或二进制片段，可能导致工具报错，现已跳过此类行
- **搜索范围**：跳过目录补充 `bin` / `lib` / `out` / `cmake-build-*` / `.idea` / `.vscode`，减少构建产物干扰
...
- **编辑文件传入空匹配串**：`edit_file` 的 old_string 为空时，会先遍历整个文件、再报出误导性的"匹配 N 次"（N 为文件长度），现在立即返回明确提示
- **无谓的网络重试**：404 / 405 / 409 / 413 / 422 这类"请求本身有问题"的错误不再重试（原先要重试满 3 次才放弃）；限流（429）与服务端错误（5xx）维持重试，其余 4xx 重试一次即止
- **响应内容被误读为状态码**：错误信息中若出现类似 `HTTP 400` 的文本（例如上游返回体里带的），可能被当成真实状态码而误判是否重试，现在只按错误开头识别
- **中断被当成网络故障**：用户中断请求时，中断状态未能传递给上层，可能被识别为连接失败并触发重试；同步请求路径也补齐了中断标志的重置
- **文件过大提示**：不再提示一个实际并不存在的 `force` 参数
