...
- [x] **5 区终端 UI** — ⚠️ 进行中（布局仍有问题，待继续）
  - 已完成：CLFConsole 原始模式输入、CLFRepl 拆分、5 区布局框架
  - 布局演进：绝对定位 → DECSTBM 滚动区 → 纯顺序输出（当前）
  - 已解决：ANSI 自检误判、UTF-8 截断乱码、输入回显重叠、确认框交互
  - **遗留问题（下次继续）**：
    1. 输入光标位置仍异常（输入内容显示错位）
    2. 窗口缩放吸底不理想
    3. 顺序输出模型下 2-5 区联动待完善
  - 提交参考：d8d5d97（5区UI）→ 4424807（光标+缩放）
- [x] **全量体检修复 4 个 bug**
