...
- **待办**：① 实机验收（第二/三波——长输出尾部结论 / not_found 归一化 / 系统提示 OS·git 信息 / 拖选 / 复制粘贴 / 中文输入 / 四域插件；输入框拖选复制——拖选高亮/松手复制/粘贴验证/单击定位无回归）② 设计文档归档（验收通过后：总排期 + 中断 + 命令层 + 平台层 → 设计/归档/）③ 版本号/发布由用户定（tag 需用户授权）④ 文案强化（中断文案加"先询问用户"——待用户拍板）
