...
- **▶ 实机回响追加修复 ✅（2026-09-23 下午，3 个提交）**：
  - Home/End 失效（fc9eecf）：ScrollView 无条件消费 → 输入框聚焦时放行给 Input（input() 字符串比较——qa 静态期教训）
  - todo 清单滞后一步（3e65afe）：jsonl 铁证模型"创建后零中途更新/update 滞后一步"——面板渲染无 bug；按用户定调"不强制教，建立清单时告诉它"：todo_write 描述加"更新要及时，尽量不要静默执行不更新"
  - **输入框拖选复制（2c2771a，新功能）**：用户 UX 定调"一个界面两套复制逻辑反人类"——FTXUI patch（InputOption.on_select 回调 + HandleMouse 三态拖选 + OnRender 通用拆段选区反色 + 非鼠标事件清选区）+ CLFRepl 装配松手复制；与显示区同机制同手感（左键拖选→高亮→松手复制），InputHandler 零改动（输入框事件本就放行）
- **▶ 输入框拖选复制三轮实机修复闭环 ✅（2026-09-23 晚）**：
  - 一轮（向上拖选失效/整行或全不选/末行丢半）：绝对基准修根（955f821）→ 二轮（复制粘贴正确、高亮错位）：含入语义 + 反向选区（a859364）→ 三轮（高亮与复制区间不一致）：**渲染级取证套件 qa_CLFInputSelection**（Screen 逐像素反色断言，不依赖终端）逐层实抓三 bug——marks 关闭 toggle 漏写（"整行高亮"真正根因，初始版就有）/ 组件 box reflect 记录分配区域非文本位置（第一行 reflect + 锚点快照修）/ while 消耗 column 致含入恒 false；反向拖锚点补含入 + 渲染复制同规则
  - **用户实机验收：完美通过**（todo.md 五场景全过）
- **▶▶ 阶段性任务全闭环 ✅✅（2026-09-23，**用户已发布 v0.9.0**）**：
  - 四波实施（中断根因 / 命令层 G2·G3·argv 化 / 平台层收敛 / P1 收尾）+ 实机回响修复（W1 残留 / D8 resume 面板 / Home·End / todo 引导 / 输入框拖选复制）全部落地
  - ctest 39/39 全绿 + --version 冒烟；用户三轮实机验收通过
  - 设计文档归档 ✅：`归档/归档-中断修复与平台收敛总排期.md`、`归档-归档-中断时效性.md`、`归档-归档-命令执行层.md`、`归档-归档-平台层收敛.md`（窄窗口渲染异常保留设计/——观察任务）
  - CHANGELOG v0.9.0 段 + VERSION bump v0.9.0 + tag v0.9.0（用户授权）
- **待办**：① thinking 治理批（等 v4.1 pro 出来再定）② 阶段 3/dsh（决策门挂起）③ 平台层第二期（Linux 三系统，§十五 矩阵待排期）④ 文案强化（中断文案加"先询问用户"——待用户拍板）

### ▶ v0.9.0 发布后实机回响 ✅✅（2026-09-23 晚，**v0.9.1 已发布**）
- **"自问自答"破案 + 双保险防御（792d83d + a170727，随 v0.9.1 发布）**：
  - 取证（C 盘发布版日志 + jsonl 严格解析）：非程序 bug——jsonl 的 user 消息全部干净、日志 [Submit] 与用户操作逐条吻合（程序零自动提交）；异常文本（`> 追问`、`用户\n指令`、假 `✻ Worked` 行）全部在 **assistant 消息内 = 模型输出**——DeepSeek 长上下文高频对话中"扮演用户"续写追问并自行执行（从历史上下文学会了 Worked 格式）
  - 修法（用户拍板双保险）：① 系统提示身份段角色边界约束（禁止模拟用户发言/模仿界面格式，"禁止+理由"措辞）② **代码层 applySelfWorkedMark**——模型自发输出 "✻ <worked> for"（CLFCode 专属格式，正常回复不可能出现）→ 尊重宣告：保留标记行、截断其后内容、丢弃 tool_calls、finishTurn 不重复拼 worked——"模型发 Worked 本身即思考异常，止损正确"（用户定调）；qa SW1/SW2（流式/同步）
  - 发布版注意：改的是代码+模板——需下次发布或手动同步 exe 与 config/system_prompt_template.md
- **E2E 自动化回归：方案先行 ✅（0b301e4），下次实施**：
  - 设计定稿：`设计/设计-Agent行为E2E自动化回归.md`（三层分工 ctest/AT/人工 + 断言原语 + 成本门（用户第一痛点：全量搜索烧钱变自动失败）+ 负控 + 环境变量隔离 CLFCODE_TEST_*（不新增 CLI 参数）+ MVP 三例 + 资产集 costGuard 成本筛查字段）；pro 定案实现形态（PowerShell runner / tools/agent-e2e/ / JSON 资产集）
  - 测试整合：`测试/测试-Agent行为实测记录.md`（用例集更名，内容唯一权威——用户已补"类型/前置"字段与 S-06/C-02 复现方式）；flash 方案稿归档（标注并入设计）
  - **待实施**（下次排期）：设计 §七 步骤 0-6——隔离钩子 → runner+断言 → 成本门 → MVP 三例 → 扩充 → 负控 → README
