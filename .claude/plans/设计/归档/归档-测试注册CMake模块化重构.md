...
> **状态**：已归档 ✅（实施完成 + 验证全过，2026-09-15，commit c48ba5f）
> **归档原因**：方案完成实施并验证——ctest 31/31 全绿（6.68s，基线 6.53s）、OFF 开关独立目录构建验证（主程序正常 + 零 qa target）、WORKING_DIRECTORY 显式固定零行为差异；版本号暂定 v0.7.5（下个版本可能为大更新，版本号届时可能调整，CHANGELOG 段已标"未发布（暂定 v0.7.5）"）
> **创建**：2026-09-11
